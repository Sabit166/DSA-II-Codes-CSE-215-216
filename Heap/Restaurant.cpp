...
public:
    ...
    Bill(char *name, int amount = 0, int timestamp = 0){
...
public:
    ...
    {
		int idxmx = find_max_update(name,1);
	    // cout<<idxmx<<endl;
		// cout<<name<<endl;
		if(idxmx!=-1)
		{
			heap[idxmx].amount = amount;
			int idxmn = find_min_update(name,1);
			heapmn[idxmn].amount = amount;
			heapifyDown(1);  
			heapifyDownmn(1);
			return;  
		}
	...
	int find_max_update(char *A,int idx)
	{
		if(string(heap[idx].name)==string(A)) 
		{
			//cerr<<idx<<endl;
			return idx;
		}
		...
		if(l<=size_ && string(heap[l].name)==string(A)) return l;
		if(r<=size_ && string(heap[r].name)==string(A)) return r;
		int ls = find_max_update(A,l);
		int rs = find_max_update(A,r);
		if(ls!=-1)
		{
			return ls;
		}
	...
	int find_min_update(char *A,int idx)
	{
		if(string(heapmn[idx].name)==string(A)) return idx;
		int l = 2*idx;
		int r = 2*idx+1;
		if(l>size_ && r>size_) return -1;
		if(l<=size_ && string(heapmn[l].name)==string(A)) return l;
		if(r<=size_ && string(heapmn[r].name)==string(A)) return r;
		int ls = find_min_update(A,l);
		int rs = find_min_update(A,r);
		if(ls!=-1)
		{
			return ls;
		}
...
//            cout<<endl;
    ...
    // Find the index of the minimum bill
    int minimumBill() 
	{
		auto mn = heapmn[1];
        cout<<mn.name<<" "<<mn.amount<<" "<<mn.timestamp<<endl;
    ...
    // Remove the bill at index i
    void DeleteIthBill(int idx) 
	{
		auto mx = heap[idx];
        int index = find_min(mx,1);
		swap(heap[size_],heap[idx]);
		swap(heapmn[size_],heapmn[index]);
        size_--;
		heapifyDown(1);
		heapifyDownmn(1);
    }
